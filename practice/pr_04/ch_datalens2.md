...
**Область исследования.** Факторы, влияющие на академическую успеваемость (GPA) старшеклассников (на основе датасета [Students Performance](https://www.kaggle.com/datasets/rabieelkharoua/students-performance-dataset/data)).
...
### Чарт 2. Участие родителей и успеваемость
...
### Чарт 4. Распределение классов в зависимости от успеваемости
*   **Тип чарта.** Кольцевая диаграмма (Donut).
*   **Цвет (Color).** `Grade_Name` (определяет разбивку на сектора).
*   **Показатели (Measures).** `Всего студентов` (определяет размер сектора).
*   **Подписи (Labels).** Перетащите сюда показатель `Всего студентов`.

Чтобы отобразить проценты, нажмите на значок шестеренки ⚙️ (настройки поля) прямо в секции «Подписи» и в поле «Значение подписи» выберите **«В процентах»** (вместо абсолютных значений). Дополнительно в «Подписи» можно перетащить и `Grade_Name`, чтобы названия классов выводились рядом с долями.
